...
```{r}
tm_shape(world_cities) +  # defines the first spatial object to plot
  tm_dots()    +       # defines how to plot the first object
tm_shape(world_vector) +  # defines the second object to plot
  tm_polygons() # defines how to plot the second object
```
...
```{r}
# fill polyon with a color palette representing a variable
tm_shape(world_vector) + 
...
tmap_save(map1, here::here("week1", "tmap-example.png"), device =ragg::agg_png )
...
```{r}
tmap_save(map1, here::here("week1", "tmap-example.jpg"))

```

```{r}
tmap_save(map1, here::here("week1", "tmap-example.html"))
